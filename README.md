...
Для инициализации библиотеки в классе `Application`, `Activity`, `View` (или любом другом с доступом к объекту `Context`) используется класс `InAppStoryManager.Builder()`. Класс содержит следующие параметры (и одноименные сеттеры).
    ...
    String userId //уникальный текстовый идентификатор пользователя (id, login, etc...), который обращается к сторис, опциональный параметр.
    ...
    String testKey //тестовый интеграционный ключ для тестирования сторис на устройстве. Опционален, по умолчанию отсутствует.
    boolean closeOnSwipe //флаг, который отвечает за закрытие сторис по смахиванию вниз. По умолчанию - true.
    boolean closeOnOverscroll //флаг, который отвечает за закрытие сторис по смахиванию влево на последней story или вправо на первом. По умолчанию - true.
    ...
    boolean hasFavorite //флаг, который отвечает за подключение функционала избранных сторис. По умолчанию - false (отключен).
    ArrayList<String> tags //теги для таргетирования сторис, опциональный параметр
...
Сам AppearanceManager содержит следующие параметры (и соответствующие им сеттеры)
    ...
    int csStoryReaderAnimation - анимация перелистывания сторис в ридере
    ANIMATION_DEPTH = 1;
    ANIMATION_CUBE = 2;
...
`IStoriesListItem csListItemInterface`, используется для полной кастомизации элементов списка.
    ...
    interface IStoriesListItem {
        ...
        void setOpened(View itemView, boolean isOpened); // itemView - текущая ячейка, меняем ее по необходимости в случае если она прочитана.
        void setHasAudio(View itemView, boolean hasAudio); // itemView - текущая ячейка, меняем ее по необходимости в случае если у данной сторис есть аудио внутри.
...
Размеры диалогового окна отображения сторис на планшете
