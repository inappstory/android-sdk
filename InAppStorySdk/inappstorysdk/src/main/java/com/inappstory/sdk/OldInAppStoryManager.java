 ...
 */
public class OldInAppStoryManager {

    private static OldInAppStoryManager INSTANCE;
    ...
    public static void useInstance(@NonNull UseOldManagerInstanceCallback callback) {
        OldInAppStoryManager manager = getInstance();
        try {
            if (manager != null) {
    ...
    public static void setInstance(OldInAppStoryManager manager) {
        synchronized (lock) {
            INSTANCE = manager;
        }
    }
        ...
        synchronized (placeholdersLock) {
            for (StoryPlaceholder placeholder : placeholders) {
                ...
                if (!this.placeholders.containsKey(key)) {
                    OldInAppStoryManager.getInstance().placeholders.put(key,
                            placeholder.defaultVal);
                }
            }
        }
    ...
    public static void initSDK(@NonNull Context context, boolean skipCheck) {
        ...
        synchronized (lock) {
            if (INSTANCE == null) {
                INSTANCE = new OldInAppStoryManager(context);
            }
        }
    ...
    private OldInAppStoryManager(Context context) {
    ...
    private void setUserIdInner(final String userId) {
        ...
        InAppStoryService.useInstance(new UseServiceInstanceCallback() {
            ...
            public void use(@NonNull InAppStoryService inAppStoryService) throws Exception {
                ...
                localOpensKey = null;
                final String oldUserId = OldInAppStoryManager.this.userId;
                OldInAppStoryManager.this.userId = userId;
                ...
                ScreensManager.getInstance().forceCloseAllReaders(
                        new ForceCloseReaderCallback() {
                            ...
                            public void onComplete() {
                                ...
                                OldInAppStoryManager.this.currentLocale = locale;
                            }
                        }
                );
                if (inAppStoryService.getFavoriteImages() != null)
    ...
    /**
     * @return current instance of {@link OldInAppStoryManager}
     */
    public static OldInAppStoryManager getInstance() {
        synchronized (lock) {
            return INSTANCE;
        }
    }
     ...
     *
     * @param tags         (tags)
     * @param outerContext (outerContext) any type of context (preferably - same as for {@link OldInAppStoryManager}
     ...
     *
     * @param context (context) any type of context (preferably - same as for {@link OldInAppStoryManager}
     * @param manager (manager) {@link AppearanceManager} for reader. May be null
     */
